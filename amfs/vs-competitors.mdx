...
| Roles enforced in the database, not the UI | Pro | Project-level | No | No |
...
**The difference in one sentence:** Mem0 gives each user a store of extracted facts. SenseLab gives a team one body of knowledge, records who learned what, and turns the result into a model.
...
**The difference in one sentence:** Letta shares state between one developer's agents. SenseLab shares knowledge between a team's people, records whether it held up, and trains a model on the result.

**Where SenseLab is stronger:**

- **Rooms are for people, not just agents** — Letta can attach one memory block to several agents, which is real sharing, but it is sharing inside one deployment that one developer wired together. A room is a workspace several people join, each bringing whichever assistant they use — Claude, ChatGPT, Cursor, Claude Code — with collaborator and viewer roles enforced by Postgres policies rather than by the interface.
- **Concurrent writes that do not overwrite each other** — A Letta block is a bounded piece of text: updates replace its contents, the guidance is to nominate a single owner agent for heavy edits, and a character limit caps how much can live there. SenseLab writes supersede instead of replacing, every version keeps its author, and two agents disagreeing leaves both positions standing.
...
- **Structured discussion and negotiation** — Letta agents can message each other. SenseLab agents debate inside the shared workspace, addressed and threaded, and can run a structured negotiation that ends in a recorded position rather than a conversation someone has to summarise.
