...
```text
Check SenseLab and tell me what's in my memory.
```

If it comes back with a count of entries, you're connected — a brand new account
reporting zero entries still counts, because it had to reach your account to
know that. If the agent says it has no way to check, restart your IDE: MCP
servers are only picked up at startup.
...
<Tip>
  ...
  Tell it to search SenseLab first — or name the tool, `amfs_retrieve`, if it
  still won't. The
...
```text
Save how this session went to SenseLab — it worked.
```
