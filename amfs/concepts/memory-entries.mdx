...
description: "The fundamental unit of knowledge in SenseLab."
icon: "file-lines"
---

A `MemoryEntry` is the fundamental unit of knowledge in SenseLab. It represents a single piece of information that an agent has recorded.
...
| `confidence` | `float` | Trust score in the range `0.0`–`1.0`, modified by outcomes (default: `1.0`) |
| `provenance` | `Provenance` | Who wrote it, when, from which session, and any `pattern_refs` |
| `shared` | `bool` | Whether other agents can see it (default: `True`). Private entries are visible only to the author |
...
| `is_artifact` | `bool` | Marks stored working files and code blobs rather than knowledge claims, so they rank below real findings |
| `branch` | `str` | Branch this version lives on (default: `"main"`) |
...
| `amfs_version` | `str` | Protocol version (currently `"0.3.0"`) |
...
Use progressive retrieval to control search scope:
...
<Note>
  Every entry is written at tier `3`. The scores that promote entries to hot or
  warm are computed by a background worker, which runs on SenseLab Cloud and on
  self-hosted SenseLab with a database backend. If you're running against local
  file or S3 storage, nothing recomputes them and `depth` is ignored — a
  `depth=1` search returns the same rows as a full one rather than a faster
  subset.
</Note>
