...
description: "How SenseLab captures decision traces that explain not just what happened, but why."
...
Agent consults external sources:
  incident tracker → 3 SEV-1 incidents in last 24h
...
```json
{
  ...
  "external_contexts": [
    {"label": "open-incidents", "summary": "3 SEV-1 incidents in last 24h", "source": "incident tracker", "recorded_at": "..."},
...
checkout-service/retry-pattern
    ...
    │   ├── also consulted: incident tracker, git log
    │   ├── wrote: decision-rollback-retry
    │   └── outcome: DEP-500 (success) → confidence reinforced
    │
    ├── read by review-agent (sess-e5f6)
    │   ├── also consulted: error tracker
    │   ├── wrote: risk-timeout-regression
    │   └── outcome: INC-1042 (critical_failure) → confidence eroded
    │
...
|:------|:------|:--------|:--------|
| Decision traces | Single session | Stored per session; Pro adds a cryptographically sealed copy | "What happened in this session and why?" |
| Knowledge graph | Cross-session, cross-agent | A persistent edge store spanning every agent on the account | "How are entities, agents, and outcomes related globally?" |

<Note>
  Both layers need a database backend, which you get on SenseLab Cloud and on
  self-hosted SenseLab configured with one. Against local file or S3 storage the
  graph calls are no-ops that return no edges and traces are not written to
  disk, so `graph_neighbors` comes back empty, multi-hop traversal with
  `depth > 1` returns nothing, and `explain()` only sees the current session.
</Note>
...
mem.record_context("open-incidents", "No active incidents", source="incident tracker")
