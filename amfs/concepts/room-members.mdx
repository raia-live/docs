...
The read-only rule is enforced in three places: the API routes, the memory write middleware, and Postgres row-level security. A viewer cannot write to a room topic by going around the room API.
