...
description: "How SenseLab treats each agent's memory as a Git repository."
icon: "code-branch"
---

SenseLab models every agent's memory as a **Git repository**. Each write, outcome, cross-agent read, and brain brief is recorded as an event on the agent's timeline — like commits in Git. This gives you full auditability, rollback capability, and branch-based collaboration.

---

## The Mental Model

When an agent is created in SenseLab, it implicitly gets a **repository** with a `main` branch. Every operation the agent performs is logged as an **event** on that branch:
...
| `read` | An entry was read | `mem.read()` |
...
| `webhook` | External context ingested via webhook | Webhook endpoint |
| `snapshot_taken` / `snapshot_recovered` | Memory snapshot created or restored | Snapshot tooling |
| `consolidation_proposed` / `consolidation_approved` / `consolidation_rejected` / `consolidation_auto_merged` | Memory consolidation activity | [Consolidation](/amfs/pro/consolidation) |
...
With SenseLab Pro, you can create branches from an agent's main timeline — just like Git branches. Branches let you:
...
The timeline is available in OSS on the `main` branch. Branching, merging, tags,
rollback, cherry-pick, fork, and access control require SenseLab Pro.

<Warning>
  The timeline needs a backend that can store events. It works as described on
  SenseLab Cloud and on self-hosted SenseLab with a database backend. Against
  local file or S3 storage, event logging is a no-op — the SDK still emits
  events, but nothing persists them and `mem.timeline()` comes back empty. If
  your timeline is unexpectedly blank, this is almost always why.
</Warning>
