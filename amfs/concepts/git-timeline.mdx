...
```bash
curl "https://amfs-login.sense-lab.ai/api/v1/agents/deploy-agent/timeline?limit=50" \
...
```bash
curl "https://amfs-login.sense-lab.ai/api/v1/entries/checkout-service/retry-pattern?branch=experiment/retry-v3" \
  -H "X-AMFS-API-Key: <your-api-key>"
```
...
The timeline records everything on `main` from your first write, with nothing to
turn on. Branching, merging, tags, rollback, cherry-pick, fork, and branch
access control are [SenseLab Pro](/amfs/editions) features.
