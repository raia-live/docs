...
how to walk immediate neighbours. This page covers what SenseLab Cloud adds:
querying the graph as a whole, and reading it as a map of expertise.

<Note>
  The persisted graph lives in Postgres. On a filesystem or S3 adapter, edges are
  not materialized and the graph tools return nothing. This is automatic on the
  hosted service.
</Note>
...
<Note>
  On SenseLab Cloud, graph results respect the same visibility rules as memory
