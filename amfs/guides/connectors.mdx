---
title: "Connectors"
description: "Feed external events into SenseLab — PagerDuty, GitHub, Slack, Jira, and custom webhooks."
icon: "plug"
---

Connectors let SenseLab ingest events from external systems and turn them into structured memory entries that agents can query and learn from.

<Note>
  Looking for the SenseLab connector inside Claude or ChatGPT? That is the other direction — memory going out to an assistant — and it is covered in [Claude and ChatGPT](/amfs/guides/claude-and-chatgpt). This page is about events coming in.
</Note>
...
**Setup:**
1. In SenseLab Dashboard → Connectors, enable **PagerDuty**
...
**Setup:**
1. In SenseLab Dashboard → Connectors, enable **GitHub**
...
**Setup:**
1. In SenseLab Dashboard → Connectors, enable **Slack**
2. Copy the webhook URL
...
**Setup:**
1. In SenseLab Dashboard → Connectors, enable **Jira**
2. Copy the webhook URL
...
SenseLab also validates the source system's signature (e.g., PagerDuty's `X-PagerDuty-Signature`, GitHub's `X-Hub-Signature-256`) using the signing secret configured in Dashboard → Connectors.
