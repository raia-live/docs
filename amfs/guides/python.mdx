---
title: "Python SDK"
description: "Complete guide to the SenseLab Python SDK."
icon: "python"
---
...
To connect to SenseLab Cloud rather than local storage, also install the HTTP
adapter:

```bash
pip install amfs-adapter-http
```
...
By default `AgentMemory` uses the filesystem adapter, so everything stays on the
local machine:
...
To reach your account, pass an `HttpAdapter`:
...
<Note>
  Setting `AMFS_HTTP_URL` on its own does not switch the SDK to the hosted
  service — the adapter argument is what decides where memory goes. The
  environment variable is read by the MCP server and the CLI, not by the SDK.
</Note>
...
When you call `commit_outcome()`, SenseLab snapshots the full decision trace — every entry that was read, every context that was recorded, and every query that was made. The resulting trace is persisted and can be retrieved later.
...
When agents call external tools or APIs, there are two ways to capture that context in SenseLab depending on your needs.
...
chain = mem.explain()
print(chain["causal_entries"])     # SenseLab entries that were read
print(chain["external_contexts"])  # tool/API inputs that informed the decision
```
...
Install the HTTP adapter and pass it to `AgentMemory`:
...
See the [Connect to SenseLab](/amfs/getting-started/saas-connection) guide for details.
