...
<Tip>
  Copy the ready-made snippet from your [SenseLab Dashboard → Agents page](https://amfs.sense-lab.ai/agents). The **MCP Connection Card** has your URL and key pre-filled.
</Tip>

<Note>
  Use `amfs-mcp-server-pro` whenever you have an API key. The plain
  `amfs-mcp-server` is the open-source build: it connects to the same API, but
  exposes only the open-source tools, so rooms, negotiations, documents, and the
  intelligence tools are missing.
</Note>
...
```bash
claude mcp add amfs \
  --env AMFS_HTTP_URL=https://amfs-login.sense-lab.ai \
  --env AMFS_API_KEY=<your-api-key> \
