...
description: "Run SenseLab in Docker or deploy to Kubernetes with Helm."
icon: "docker"
---

<Note>
This guide is for self-hosted deployments. Using the managed platform? You don't need Docker — see [Connect to SenseLab](/amfs/getting-started/saas-connection).
</Note>
...
SenseLab ships with a production-ready Docker image and a Helm chart for Kubernetes. You can go from zero to a running SenseLab server in one command — no Python installation required.
...
|:--------|:-----|:------------|
| `amfs` | 8080 | SenseLab HTTP API server |
...
|:------|:------------|:--------|
| `replicaCount` | Number of SenseLab pods | `1` |
...
| `amfs.namespace` | SenseLab namespace | `default` |
