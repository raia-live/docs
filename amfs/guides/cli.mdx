...
<Note>
The CLI is for self-hosted AMFS. On the managed platform, use the [Dashboard](https://amfs-login.sense-lab.ai) or [REST API](/amfs/guides/http-server) instead.
</Note>

The AMFS CLI provides commands for inspecting, diffing, and snapshotting local memory from the terminal.
...
See the [Connect to AMFS](/amfs/getting-started/saas-connection) guide and [Environment Variables](/amfs/reference/environment-variables) for details.
