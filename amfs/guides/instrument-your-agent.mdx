...
| Outcome | `amfs.outcome_type` on the root, else the root span's status: `OK` → success, `ERROR` → failure |
...
amfs_commit_outcome("deploy-142", "success", task_input="roll api back to v41")
```
...
<Tip>
  ...
  with `amfs_record_llm_call`, close spans with `amfs_end_span`."
</Tip>

Requires `amfs-mcp-server-pro` 0.1.53 or later.
