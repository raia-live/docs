...
Returns the causal chain for the current session: which SenseLab entries were read, which external contexts were recorded, and their details. If `outcome_ref` is provided, labels the explanation with that reference.
...
```python
{
    ...
    "causal_entries": list[dict],       # SenseLab entries that were read
...
Link memory entries to external blobs — model weights, datasets, logs, screenshots, or any binary artifact stored outside SenseLab.
...
The following tools are available only with the SenseLab Pro MCP server.
