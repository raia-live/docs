...
Automatically ingest events from external systems into SenseLab memory. The connector framework transforms raw events into SenseLab `write()` or `record_context()` operations, with HMAC verification, deduplication, and pluggable transforms.
