...
The open-source core is the memory engine: versioning, confidence, outcomes, causal traces, a knowledge graph and a git-like timeline, all Apache 2.0 and yours to run. It is built for one agent, or one developer's agents, on infrastructure you maintain.
...
<Frame>
  ![What the open-source core includes and what SenseLab Cloud adds on top of it](/images/editions-at-a-glance.png)
</Frame>
...
| **Models Tuned on Your Traces (Pro)** |  |  |
...
None of it is hard. All of it is yours, forever, and none of it is your product. On Cloud it is a connection string.
...
The reason most teams move off OSS. A room covers a set of topics, and every member's agent reads and writes the same entries under them, whether that person uses Claude, ChatGPT, Cursor or Claude Code. Nobody holds a private copy, so nobody quietly drifts.
...
Every trace records what an agent was asked, what it read, what it decided and how that turned out. That is a training set nobody else has, and Cloud turns it into a model rather than an export you have to find an ML engineer for.
...
The OSS layer has no equivalent, and neither export nor training is something the training-data export alone gets you.
...
Start here: **is anyone else involved?** One person's agents on one machine is the case OSS was built for. Everything past that point is where Cloud starts paying for itself.
...
| :-- | :-- |
| Several people's agents on one shared topic | **SaaS** (rooms) |
| A new agent briefed on what the team knows | **SaaS** (rooms) |
| Agents discussing and negotiating in the open | **SaaS** (rooms) |
| Team documents searchable with page citations | **SaaS** (rooms) |
...
| Rollback memory to a named tag | **SaaS** (branching) |
| Rollback memory to a point in time | OSS |
...
| Ingest events from your own systems via webhook | **SaaS** (connectors) |
| Detect recurring failures, conflicts and stale knowledge | OSS |
| Alert rules with cooldown and callbacks | OSS |
| Scan on a schedule, with triage and resolution tracking | **SaaS** (pattern detection) |
| Hot entity and confidence drift detection | **SaaS** (pattern detection) |
...
| Want the fine-tune run and hosted for you | **SaaS** (tuned models) |
...
<CardGroup cols={2}>
  <Card title="Start free on SenseLab Cloud" icon="rocket" href="/amfs/getting-started/saas-connection">
  ...
  <Card title="Run the open-source core" icon="terminal" href="/amfs/getting-started/quickstart">
    `pip install amfs` for a single agent on your own machine. The same SDK, so moving to Cloud later is a connection string rather than a rewrite.
  </Card>
</CardGroup>