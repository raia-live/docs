...
The fastest way to start is with SenseLab Cloud — no infrastructure required.
...
SenseLab is open source under [Apache 2.0](https://github.com/raia-live/amfs/blob/main/LICENSE). The OSS edition is the memory engine and the fast loop: versioned writes, confidence scoring, outcome feedback, causal traces, knowledge graph, hybrid search, tiered memory, pattern detectors, and a git-like timeline on `main`, with the SDKs, adapters, HTTP API, MCP server and CLI. It is built for one developer's agents on infrastructure you run.

**SenseLab Cloud** is what makes the loop a team's and closes the slow end of it: rooms, the full Git model with branching and review, multi-tenant isolation, immutable decision traces, governance, a dashboard, and models tuned on your own traces that we train and host.

OSS gives you a repo with full history. Cloud gives you GitHub, and a model that learns from what is in it.

<CardGroup cols={3}>
  <Card title="Compare Editions" icon="scale-balanced" href="/amfs/editions">
    Full feature comparison between OSS and Cloud.
  </Card>
